...
describe('lazy splitLines round-trip', () => {
  const mixedCalls = (): Record<string, SnapshotCall> => {
    ...
    return {
      ...
      'q:2': { args: [serialize(mixed)], result: serialize(largeValue()) },
      'q:3': { args: [], result: serialize([1, 2, 3]) },
    };
  };
  ...
  const roundTrip = (gzip: boolean): void => {
    ...
    withEnv(
      { ...SMALL_POOL_LIMITS, UNIMOCK_GZIP: gzip ? '1' : undefined },
      () => {

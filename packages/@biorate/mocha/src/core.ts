...
export abstract class ClassTestUI {
  ...
  private static readonly nameForParametersSymbol =
    ClassTestUI.MakeSymbol('nameForParameters');
  ...
  private static readonly parallelSymbol = ClassTestUI.MakeSymbol('parallel');
  ...
  public readonly parallel: ExecutionOptionDecorator;
  ...
  public constructor(runner: TestRunner) {
    ...
    this.parallel = this.createExecutionOption(ClassTestUI.parallelSymbol);
  ...
  private getSettings(obj: any): LifecycleSettings | TestSettings | SuiteSettings {
    ...
    if (ClassTestUI.parallelSymbol in obj) {
      (settings || (settings = {})).parallel = obj[ClassTestUI.parallelSymbol];
    }
    return settings;
  }
  ...
  private suiteCallbackFromClass<T extends TestInstance>(
    constructor: TestClass<T>,
  ): () => void {
    ...
    return function () {
      ...
      if (prototype.after) {
        if (isAsync(prototype.after)) {
          theTestUI.runner.afterEach(
            'after',
            wrap(function (done) {
              instance[theTestUI.context] = this;
              return prototype.after.call(instance, done);
            }, prototype.after),
            theTestUI.getSettings(prototype.after),
          );
        } else {
          theTestUI.runner.afterEach(
            'after',
      ...
      if (constructor.after) {
        if (isAsync(constructor.after)) {
          theTestUI.runner.afterAll(
            'static after',
            wrap(function (done) {
              constructor[theTestUI.context] = this;
              return constructor.after(done);
            }, constructor.after),
            theTestUI.getSettings(constructor.after),
          );
        } else {
          theTestUI.runner.afterAll(
            'static after',
  ...
  private makeSuiteFunction(execution?: Execution): SuiteDecoratorOrName {
    ...
    const decorator = function () {
      ...
      if (
        ...
        !arguments[0][ClassTestUI.isDecoratorSymbol]
      ) {
      ...
      function applySuiteDecorator(name: string, ctor) {
        ...
        theTestUI.runner.suite(
          name,
          theTestUI.suiteCallbackFromClass(ctor),
          theTestUI.getSettings(ctor),
        );
      }
    };
  ...
  private makeParamsFunction(execution?: Execution) {
    return function (params: any, name?: string) {
      return function (target: Object, propertyKey: string) {
        target[propertyKey][ClassTestUI.nameSymbol] = propertyKey.toString();
        target[propertyKey][ClassTestUI.parametersSymbol] =
          target[propertyKey][ClassTestUI.parametersSymbol] || [];
        target[propertyKey][ClassTestUI.parametersSymbol].push({
          execution,
          name,
          params,
        } as TestParams);
  ...
  private createExecutionOption(key: symbol | string): ExecutionOptionDecorator {
    ...
    return function (value: number | boolean): ClassDecorator & MethodDecorator {
...
export type CallbackOptionallyAsync = (
 ...
 */
export type ExecutionOptionDecorator = (
  value: number | boolean,
) => ClassDecorator & MethodDecorator & RelaxedMethodDecorator;

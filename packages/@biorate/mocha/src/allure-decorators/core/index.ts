  ...
  return (target: unknown, propertyKey: string, descriptor: PropertyDescriptor) => {
    ...
    if (typeof original === 'function') {
      descriptor.value = function (...args: [T]) {
        try {
          const value: string =
            typeof nameFn === 'function' ? nameFn.apply(this, args) : nameFn;
...
export const testCaseId = <T>(idFn: string | ((arg: T) => string)): TestDecorator => {
  return processDecorator(idFn, (id) =>
    getAllure().link(`${getTmsUrl()}/${id}`, id, LinkType.TMS),
  );
};
...
export const description = <T>(descriptionFn: string | ((arg: T) => string)) => {
  return processDecorator(descriptionFn, (text) => {
    getAllure().description(text);
  });
};

export const label = (name: string, value: string) => {
  return processDecorator(name, () => {
    getAllure().label(name, value);
  });
};
